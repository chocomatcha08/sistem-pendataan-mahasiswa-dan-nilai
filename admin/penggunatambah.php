...
if (isset($_POST['tambah'])) {
  ...
  if (empty($nama_user) || empty($password) || empty($nama) || empty($level) || empty($status)) {
    echo "<script>alert('Semua field harus diisi.'); document.location.href = 'pengguna.php';</script>";
    exit;
  }

  // Check if username already exists
  $stmt = $db->prepare("SELECT * FROM user WHERE nama_user = ?");
  ...
  $stmt = $db->prepare("INSERT INTO user (nama_user, password, nama, level, status) VALUES (?, ?, ?, ?, ?)");
  $stmt->bind_param("sssss", $nama_user, $hashed_password, $nama, $level, $status);
            ...
            <div class="container mt-5">
                ...
                <form action="" method="POST">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <small id="passwordHelp" class="form-text text-muted">Silakan masukkan password
                            lengkap Siswa</small>
                    </div>
                    <div class="form-group mb-3">
